...
public class ceProperties {
	...
	public ceProperties(File file) {
		...
		try {
			...
			CraftEssence.log.log(Level.SEVERE,
					"[CraftEssence]: Cannot read file " + file.getName());
		} catch (IOException e) {
			CraftEssence.log.log(Level.SEVERE,
					"[CraftEssence]: Cannot create file " + file.getName());
		}
	}
	...
	public int getInt(String key, Integer defaultValue, String defaultComment) {
		if (map.containsKey(key)) {
			try {
				...
				CraftEssence.log.log(Level.WARNING,
						"[CraftEssence]: Trying to get Integer from " + key
								+ ": " + map.get(key).value);
				return 0;
			}
		} else {
	...
	public double getDouble(String key, Double defaultValue,
		...
		if (map.containsKey(key)) {
			try {
				...
				CraftEssence.log.log(Level.WARNING,
						"[CraftEssence]: Trying to get Double from " + key
								+ ": " + map.get(key).value);
				return 0;
			}
		} else {
	...
	public void save() {
		...
		try {
			...
			CraftEssence.log.log(Level.SEVERE,
					"[CraftEssence]: IO Exception with file " + file.getName());
		} finally {
			try {
				if (bwriter != null) {
					bwriter.flush();
				...
				CraftEssence.log.log(
						Level.SEVERE,
						"[CraftEssence]: IO Exception with file "
								+ file.getName() + " (on close)");
			}
		}
